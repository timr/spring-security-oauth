...
public class BufferedImageHttpMessageConverter implements HttpMessageConverter<BufferedImage> {
	...
	/**
	 * This constructor differs from Spring's version in order to get around
	 * some weird registered image types.
	 */
	public BufferedImageHttpMessageConverter() {
		String[] readerMediaTypes = ImageIO.getReaderMIMETypes();
		for (String mediaType : readerMediaTypes) {
			if(null==mediaType || "".equals(mediaType)) continue;
		...
		if (writerMediaTypes.length > 0) {
			this.defaultContentType = MediaType.parseMediaType(writerMediaTypes[1]);
		}
	}
